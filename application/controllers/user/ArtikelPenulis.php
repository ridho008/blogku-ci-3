...
class ArtikelPenulis extends CI_Controller {
	...
	{
		$data['title'] = 'Penulis Artikel | ' . $this->session->userdata('username');
		$data['user'] = $this->db->get_where('users', ['username' => $this->session->userdata('username')])->row_array();

		// Ambil DB penulis untuk mengambil id penulis
		$where = [
			'id_user' => $this->session->userdata('id_user')
		];
		$dataPenulis = $this->Artikel_model->getWhereCustom($where)->row_array();

		// Ambil DB artikel berdasarkan id penulis
		$where = [
			'penulis.id_penulis' => $dataPenulis['id_penulis']
		];
		$data['artikel'] = $this->Artikel_model->getWhereUserPenulis($where);
		// var_dump($data['artikel']); die;
		$this->load->view('layout/header', $data);
		$this->load->view('layout/navbar', $data);
		$this->load->view('penulis/artikel/index', $data);
		$this->load->view('layout/footer');
	}

	public function tambahArtikel()
	{
		$data['title'] = 'Tambah Data Artikel | ' . $this->session->userdata('username');
		$data['user'] = $this->db->get_where('users', ['username' => $this->session->userdata('username')])->row_array();
		...
		if($this->form_validation->run() == FALSE) {
			$this->load->view('layout/header', $data);
			$this->load->view('layout/navbar', $data);
			$this->load->view('penulis/artikel/tambah_artikel', $data);
			$this->load->view('layout/footer');
		} else {
	...
	public function ubahArtikel($id)
	{
		...
		if($this->form_validation->run() == FALSE) {
			$this->load->view('layout/header', $data);
			$this->load->view('layout/navbar', $data);
			$this->load->view('penulis/artikel/ubah_artikel', $data);
			$this->load->view('layout/footer');
		} else {
			if($this->input->post('1') !== null) {
				$status = '1';
